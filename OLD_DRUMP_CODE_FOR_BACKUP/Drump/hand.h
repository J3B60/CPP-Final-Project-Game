...
class Hand {
	public:
		...
		bool MonsterTribute(int HandPosition, int ForceZoneInput); //Selecting the ForceZone and Monster for Tribute in the Hand
		bool DrawCardToHand(Card DrawnCard);
		vector<Card> PlayerHand = {};	//Holds the Players Hand
	private:
		//vector<Card> PlayerHand = {};	//Holds the Players Hand
		Card EMPTY; //NULL CARD
		int CurrentCardPosition = 0; //The Current Chosen Position, Requires 2 functions to be run hence use of variable, Set to 0 at the end of Last Function
};
